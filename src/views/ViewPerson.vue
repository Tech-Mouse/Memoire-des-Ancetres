<script setup>
    ...
    const fetchPerson = async (id) => {
        try {
            ...
            const resParents = await fetch(`http://localhost:3000/api/people/${id}/parents`);
            parents.value = await resParents.json();
        
            const resSpouses = await fetch(`http://localhost:3000/api/people/${id}/spouses`);
            if (resSpouses.ok) {
            ...
            const resCategories = await fetch(`http://localhost:3000/api/people/${id}/categories`);
            categories.value = await resCategories.json();

            const resChildren = await fetch(`http://localhost:3000/api/people/${id}/children`);
    ...
    watch(() => route.params.person_id, (newId) => {
...
<template>
    <div v-if="person">
        ...
        <PersonDetails :person="person" :parents="parents" :spouses="spouses" :categories="categories" :children="children" />
    </div>
</template>